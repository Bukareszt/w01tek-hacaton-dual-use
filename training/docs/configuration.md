...
| `flat_quiet_v11_sticky_dr` | flat_quiet_baseacc | One knob: `dr.foot_friction.range=[0.4,1.8]` (effective mu 0.36-1.62, up from 1.22) so the draw covers rubber on rough concrete and carpet, where the deployed policy stumbles. 400M keeper-restore budget. Gate on the full 25-row courses table vs the baseacc keeper: sticky rows up, nothing else down >10%, impact proxy flat, spins wz >= 0.8, slow 0.25 still stepping. |
