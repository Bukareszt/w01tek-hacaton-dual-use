...
Parametry: `policy_dir`, `joint_map_yaml`, `imu_mount_rpy` (rotacja IMU→base_link; real: `[0, π, 0]`, sim: zera), `clamp_knee`, `auto_enable`, `soft_start_s` (blend od pozy mierzonej do wyjścia polityki), `watchdog_timeout_s` (0.2 s — przy nieświeżych danych **wstrzymuje publikację**; MD80 trzyma ostatni cel), `cmd_vel_timeout_s` (domyślnie `0` = wyłączone, ostatnia komenda trzymana; `> 0` — komenda starsza niż to zeruje prędkości, wysokość stania zostaje; stos nawigacyjny startuje robota z `cmd_vel_timeout_s:=0.5`, bo `teleop_twist_keyboard` i panel Teleop Foxglove publikują tylko na naciśnięcie i polegają na trzymaniu), `gravity_from_accel` (filtr komplementarny zamiast kwaternionu IMU, dla IMU bez fuzji).

Zabezpieczenia: watchdog świeżości per-sensor (polityka bez IMU przeżywa dropout IMU), opcjonalny dead-man na `/cmd_vel` (`cmd_vel_timeout_s`; martwy nadawca = stanie w miejscu, nie marsz na ostatniej komendzie), odrzucanie NaN w wejściach (NaN zatruwałby stan przez pętlę `last_action`), reset stanu polityki na krawędzi hold→run.
